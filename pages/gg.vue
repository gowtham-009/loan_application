    ...
    <div class="w-100" :style="{ height: boxex2Height + 'px' }">
      ...
      <div class="w-100 pa-1" :style="{ height: form + 'px' }">
        ...
        <div v-if="error" class="w-100">
          <v-alert title="Error" type="danger">
            {{ error }}
          </v-alert>
        </div>
        <div v-else>
          <v-form ref="form" :style="{ height: form + 'px' }" v-model="isValid" @submit.prevent="handleventfun"
            class="w-100 d-flex justify-space-between flex-column">

            <div class="w-100 scroller">
              <v-text-field v-model="mobile_no" :counter="10" :rules="mobilerules" label="Mobile Number"
                placeholder="Enter Mobile Number" type="number" hide-details required class="w-100" maxlength="10"
                @input="checkLength"></v-text-field>
              ...
              <v-row class="d-flex flex-column ga-0" style="margin-top: 2px;">
                <v-col cols="12">
                  <v-text-field v-model="proofid" :counter="10" :rules="proofRules" label="Proof ID / Number"
                    placeholder="Enter Unique ID Number" hide-details required class="w-100"
                    @input="proofid = proofid.toUpperCase()"></v-text-field>
                </v-col>
                  ...
                  <div v-if="imageshow2" class="w-100 pa-2">
                    <div class="w-100 d-flex flex-column">
                      ...
                      <div class="camera-container d-flex flex-column align-center justify-center">
                        <v-btn color="#33691E" width="330" prepend-icon="mdi mdi-camera" height="200"
                          v-if="!cameraActive" @click="openCamera">
                        ...
                        <div v-if="cameraActive" class="credit-card-container">
                          ...
                          <template v-else>
                            <video ref="video" class="video-feed" autoplay playsinline
                        ...
                        <div v-if="cameraActive" class="controls">
                          <div class="w-100 d-flex ga-3 justify-center align-center pa-4">
                            <div class="w-100">
                              <v-btn block :color="capturedImage ? '#0277BD' : '#33691E'"
                                prepend-icon="mdi mdi-camera-flip-outline" variant="flat"
                                @click="capturedImage ? openDialog() : switchCamera">
                                {{ capturedImage ? 'Expand' : 'Flip' }}
                              </v-btn>
                            </div>
                            <div class="w-100">
                              <v-btn variant="tonal" block prepend-icon="mdi mdi-camera-enhance" v-if="!capturedImage"
                                @click="captureImage">
                                Capture Image
                              </v-btn>
                              <v-btn block class="flex-grow-1" variant="flat" prepend-icon="mdi mdi-camera-retake"
                                v-if="capturedImage" @click="retakeImage">
                                Re-Capture Image
                              </v-btn>
                            </div>
                          </div>
